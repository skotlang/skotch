//! skot Kotlin Library (.klib) writer and reader.
//!
...
//! ```
//!
//! `skot` mirrors that pipeline shape so the same fixture inputs we
...
//!                            ^                ^         ^
//!                            │                │         │
//!                      this crate    skot-backend-llvm  driver
//! ```
//!
//! ## Format
//!
//! A skot `.klib` is a ZIP archive (matching kotlinc's choice). Its
...
//!             module                module name
//!         ir/
//!             module.skir.json      serialized MirModule (skot's IR)
...
//! a JSON encoding of skot's [`MirModule`]. Round-tripping a skot klib
//! through [`read_klib`] and back through [`write_klib`] yields a
//! byte-equal file (modulo zip header timestamps, which we pin).
//!
//! The skot klib is **not interchangeable** with a kotlinc-native
...
impl KlibManifest {
    /// Default manifest produced for skot's klibs. The `compiler` key
    ...
    pub fn parse(text: &str) -> Result<Self> {
        ...
        Ok(KlibManifest {
            ...
            compiler: compiler.unwrap_or_else(|| "skot".to_string()),
...
/// Read a skot `.klib` and return the deserialized [`MirModule`] +
...
pub fn read_klib(bytes: &[u8]) -> Result<(MirModule, KlibManifest)> {
    ...
    if manifest.compiler != "skot" {
        return Err(anyhow!(
            "klib was produced by `{}`, not `skot` — only skot klibs are readable",
            manifest.compiler
        ));
    }
    ...
    let module: MirModule =
        serde_json::from_slice(&json).context("deserializing skot MIR from klib")?;
    Ok((module, manifest))
}
...
/// `skot`). Useful for `xtask` so it can refuse to mistake a kotlinc
/// klib for a skot one.
...
mod tests {
    ...
    fn klib_round_trip_preserves_module() {
        ...
        assert_eq!(manifest.compiler, "skot");
