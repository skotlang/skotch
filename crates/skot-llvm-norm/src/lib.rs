...
//!
//! Two independent compilers (skot and `kotlinc-native` + `clang -S
...
//! that captures *what skot itself emits*. This isn't a faithful
...
//!   registers in PR #4 — they're already deterministic in skot's
...
//! - Stripping `dbg !N` annotations from instruction lines (skot
//!   doesn't emit them)
