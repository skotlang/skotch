...
//!
//! For each supported fixture, run `skot emit --target dex` and then
...
#[test]
fn skot_dex_files_pass_dexdump() {
    ...
    for &name in SUPPORTED {
        let input = workspace_root()
            ...
            .join("input.kt");
        let tmp =
            std::env::temp_dir().join(format!("skot-e2e-dex-{}-{}", name, std::process::id()));
