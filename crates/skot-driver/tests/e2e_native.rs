...
//!
//! For each supported fixture, run `skot emit --target native`
...
#[test]
fn skot_native_binaries_match_committed_stdout() {
    ...
    for &name in SUPPORTED {
        ...
        let tmp =
            std::env::temp_dir().join(format!("skot-e2e-native-{}-{}", name, std::process::id()));
...
#[test]
fn skot_native_matches_kotlinc_native_observable_behavior() {
    ...
    for &name in SUPPORTED {
        let dir = workspace_root()
            .join("tests/fixtures/expected/native")
            .join(name);
        let skot = dir.join("run.stdout");
        let kn = dir.join("kotlinc-native.run.stdout");
        if !skot.exists() || !kn.exists() {
            continue;
        }
        let skot_text = std::fs::read_to_string(&skot).unwrap();
        let kn_text = std::fs::read_to_string(&kn).unwrap();
        if skot_text != kn_text {
            failures.push(format!(
                "{name}: skot vs kotlinc-native stdout differ\n  skot: {skot_text:?}\n  kn:   {kn_text:?}"
            ));
        }
    }
    if !failures.is_empty() {
        panic!(
            "{} fixture(s) have skot/kotlinc-native stdout drift:\n  - {}",
            failures.len(),
            failures.join("\n  - ")
        );
    }
