//! Bidirectional type checker for the Kotlin subset skot accepts.
//!
...
use skot_diagnostics::{Diagnostic, Diagnostics};
use skot_intern::Interner;
use skot_resolve::{DefId, ResolvedFile};
use skot_syntax::{
    BinOp, Block, Decl, Expr, FunDecl, KtFile, Stmt, TemplatePart, TypeRef, ValDecl,
};
use skot_types::{ty_from_name, Ty};
...
impl<'a> TypeChecker<'a> {
    ...
    fn check_function(
        ...
        idx: u32,
        f: &FunDecl,
        rf: &skot_resolve::ResolvedFunction,
    ) -> TypedFunction {
        ...
        let mut scope: Vec<(skot_intern::Symbol, Ty)> = Vec::new();
    ...
    fn check_block(
        &mut self,
        block: &Block,
        scope: &mut Vec<(skot_intern::Symbol, Ty)>,
