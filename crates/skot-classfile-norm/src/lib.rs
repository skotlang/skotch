...
//! ## Why this exists
//!
//! `skot` and `kotlinc` are two independent compilers, so byte-level
...
//! match between skot and kotlinc for the PR #1 fixtures.
//!
...
//!   they reference
//! - **Minor version:** zeroed (kotlinc and skot agree but other JVM
...
//!   — punted until skot can produce them.
        ...
        while i < count {
            ...
            let entry = match tag {
                1 => {
                    ...
                    // Modified UTF-8 vs UTF-8: skot's PR #1 only emits
...
mod tests {
    ...
    fn minimal_class() -> Vec<u8> {
        ...
        // golden tests live in skot-backend-jvm.
