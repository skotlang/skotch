//! Architectural test: the shipping `skot` binary must contain none
//! of the strings `kotlinc`, `javac`, `d8`, or `dx`.
//!
//! This enforces the hard rule that skot does not invoke external
...
//!
//! If skot ever calls `Command::new("kotlinc")`, the literal `"kotlinc"`
...
/// Strings the shipping skot binary must NOT contain.
///
...
#[test]
fn skot_binary_contains_no_external_compiler_names() {
    ...
    if !hits.is_empty() {
        let report: Vec<String> = hits
            ...
            .collect();
        panic!(
            "skot binary contains forbidden tool name(s):\n{}\n\nThe shipping skot binary must not invoke kotlinc/javac/d8/dx. \
